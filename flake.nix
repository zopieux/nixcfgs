{
  ...
  outputs = { self, nixpkgs, ... }@inputs: {
    ...
    homeConfigurations = let hive = (inputs.colmena.lib.makeHive self.colmena);
    in with nixpkgs.lib;
    listToAttrs (flatten (mapAttrsToList (machine: machineConfig:
      (mapAttrsToList (user: homeConfig: ({
        name = "${user}@${machine}";
        value = homeConfig;
      })) machineConfig.config.home-manager.users)) hive.nodes));
